...
function translateText(inputText, fromLang, toLang) {
    ...
    fetch(apiUrl).then(response => response.json()).then(data => {
        ...
        document.getElementById('outputText').innerText= "Error: An error occurred";
    });
}
...
function speakText(text){
    ...
    const speechUtterrance = new SpeechSynthesisUtterance(text);
    speechUtterrance.lang = document.getElementById('translateTo').value;

    speechSynthesis.speak(speechUtterrance);
}
